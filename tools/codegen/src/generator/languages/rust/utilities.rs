...
static RUST_KEYWORDS: OnceLock<HashSet<String>> = OnceLock::new();

fn rust_keyword() -> &'static HashSet<String> {
...
fn avoid_keywords(name: &str) -> String {
    ...
    if keywords.contains(&snake_name) {
        snake_name + "_"
    } else {
        snake_name
    }
...
pub(crate) fn field_name(name: &str) -> m4::Ident {
    let span = m4::Span::call_site();
    m4::Ident::new(&avoid_keywords(name), span)
}
